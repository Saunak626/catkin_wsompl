...
class ValidityChecker : public ob::StateValidityChecker//状态有效检测
{
public:
  ...
  bool isValid(const ob::State* state) const
  {
    ...
    // 我们知道在此示例中我们正在使用RealVectorStateSpace，因此我们将状态转换为特定类型。
    ...
    /*
        ...
        */
    double x = state3D->values[0];//获取坐标
...
// 传入pathFinding的是start_pt，target_pt
void pathFinding(const Vector3d start_pt, const Vector3d target_pt)
{
  ...
  /*
    STEP 4: Create a problem instance, please define variable as pdef
    */
  ob::ProblemDefinitionPtr pdef(new ob::ProblemDefinition(si));//为问题实例设置优化的目标函数
  //可能有问题
